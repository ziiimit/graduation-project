...
def spider(url):
    ...
    # 获取视频title
    title = driver.find_element(By.XPATH, "//div[@id='above-the-fold']/div[@id='title']/h1").text
...
def getTranscriptSegments(vedioID,timestamps):
    transcripts = YouTubeTranscriptApi.get_transcript(vedioID)
...
def main():
    ...
    vedioURL = "https://www.youtube.com/watch?v=zVCaYyUWWSw"
    theme = 'focus_productivity_and_creativity'

    vedio_info = spider(vedioURL)
    vedioTitle = vedio_info['vedioTitle']
    timestamps = vedio_info['timestamps']

    vedioID = vedioURL.split('=')[-1]
    segments = getTranscriptSegments(vedioID=vedioID,timestamps=timestamps)
    ...
    path = PROJECT_ROOT + f'/data/raw_transcript/{theme}/{vedioTitle}.json'
